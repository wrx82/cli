	...
	tests := []struct {
	...
	}{
		{
			...
			want: DownloadOptions{
				...
				DoPrompt:       true,
				Names:          []string(nil),
				DestinationDir: mustCanonicalise("."),
			},
		},
		{
			name:  "with run ID",
		...
		{
			name:  "to destination",
			args:  fmt.Sprintf("2345 -D %s", tmpDir),
			isTTY: true,
			want: DownloadOptions{
				RunID:          "2345",
				DoPrompt:       false,
				Names:          []string(nil),
				DestinationDir: mustCanonicalise(tmpDir),
			},
		},
		{
		...
		for _, testArtifact := range run.testArtifacts {
			if testArtifact.artifact.DownloadURL == url {
				for _, file := range testArtifact.files {
					path, err := dir.Join(file, filepaths.MissingOk)
					if err != nil {
						return err
					}
					return os.WriteFile(path.String(), []byte{}, 0600)
				}
			}
		}
	}
...
func Test_runDownload(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			opts: DownloadOptions{
				RunID:          "2345",
				DestinationDir: mustCanonicalise("./tmp"),
			},
			platform: &fakePlatform{
				runs: []run{
					{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			opts := &tt.opts
			if opts.DestinationDir == nil {
				opts.DestinationDir = mustCanonicalise(t.TempDir())
			} else {
				opts.DestinationDir = mustCanonicalise(filepath.Join(t.TempDir(), opts.DestinationDir.String()))
			}
			...
			require.Equal(t, len(tt.expectedFiles), countFilesInDirRecursively(t, opts.DestinationDir.String()))

			// Then check that the exact files are correct
			for _, name := range tt.expectedFiles {
				require.FileExists(t, filepath.Join(opts.DestinationDir.String(), name))
			}
